...
def valence_weight(valence_new: int) -> int:
...
def compute_face_center(vertices: np.ndarray, faces: np.ndarray) -> np.ndarray:
    return np.sum(vertices[faces], 1) / 3.0

def compute_edges(faces: np.ndarray) -> list[tuple[int, int]]:
    edges = set()
    for face in faces:
        for i in range(3):
...
def compute_vertex_mappings(vertices: np.ndarray, faces: np.ndarray, edges: list[tuple[int, int]]) -> tuple[list[set[int]], list[set[int]]]:
...
def simplify(vertices: np.ndarray, faces: np.ndarray, target_v: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    face_centers = compute_face_center(vertices, faces)
    edges = compute_edges(faces)
    v2v, v2f = compute_vertex_mappings(vertices, faces, edges)
    ...
    for i, (a, b) in enumerate(edges):
        ...
        try:
            ...
            v4_new = np.r_[0.5 * (vertices[a] + vertices[b]), 1]
    ...
    vertex_mask = np.ones(len(vertices), dtype=bool)
    face_mask = np.ones(len(faces), dtype=bool)
    merges = [{i} for i in range(len(vertices))]
    ...
    while np.sum(vertex_mask) > target_v and len(E_heap) > 0:
        ...
        if vertex_mask[a] and vertex_mask[b]:  # skip if either vertex is already removed
            if len(v2v[a] & v2v[b]) == 2 and len(v2f[a] & v2f[b]) == 2:  # skip non-manifold and boundary edges
                collapse_edge(vertices, a, b, vertex_mask, face_mask, merges, Q, E_heap, v2v, v2f)
...
def collapse_edge(vertices, a, b, vertex_mask, face_mask, merges, Q, E_heap, v2v, v2f):
    ...
    # recompute E
    for c in v2v[a]:
        v4_mid = np.r_[(vertices[a] + vertices[c]) / 2, 1]
        Q_new = Q[a] + Q[c]
